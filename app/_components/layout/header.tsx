    ...
    <header className="sticky top-0 z-50 w-full border-b bg-white/90 backdrop-blur-sm dark:bg-gray-900/90 dark:border-gray-800">
      <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center space-x-6 md:flex">
          {navItems.map((item) => (
            <Link key={item.name} href={item.href} className="text-sm font-medium text-gray-700 dark:text-gray-300 transition-colors hover:text-lime-600">
              {item.name}
            </Link>
          ))}
          <Link href={process.env.NEXT_PUBLIC_BLOG_URL || '#'} target="_blank" rel="noopener noreferrer" title="마케팅파크 공식 블로그" className="text-gray-500 dark:text-gray-400 hover:text-lime-600">
            <Rss size={20} />
          </Link>
          <Link href={process.env.NEXT_PUBLIC_YOUTUBE_URL || '#'} target="_blank" rel="noopener noreferrer" title="마케팅길잡이 (구독자 2만명)" className="flex items-center space-x-1 text-gray-500 dark:text-gray-400 hover:text-lime-600">
            <Youtube size={20} />
